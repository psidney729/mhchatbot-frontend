...
const ChatbotLLM: React.FC = () => {
  ...
  return (
    <Container maxWidth="lg" style={{ height: '86vh', display: 'flex', flexDirection: 'column' }} sx={{
      pt: 2,
      pb: 0,
    }}>
        ...
        {messages.map((message, index) => (
          ...
          >
            {message.role === 'assistant' && (
              <Avatar alt="Assistant" style={{ marginRight: '8px' }}>A</Avatar>
            )}
            <MessageBubble
              elevation={1}
              style={{
                backgroundColor: message.role === 'user' ? '#d1e7ff' : '#e9ecef',
              }}
            >
